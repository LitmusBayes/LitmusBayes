...
# BAYES_FILE = '../experiment/RQ2/log_record_best_post_final_kmeans_10_factor_2_banana.log.validation_cache.jsonl'
# BASELINE_FILE = '../experiment/RQ2/final_results/init/median_results.json'
BASELINE_FILE = '../experiment/RQ2/final_results/init/median_results.json'
BAYES_FILE = '../experiment/RQ2/final_results/post_final/median_results.json'  # Added Second Bayes File
...
for litmus, score in bayes_data.items():
    if score > 0:
        times += 1
print(times)
for litmus, s_base in baseline_data.items():
    ...
    if has_positive and not has_neg_one:
        ...
        s_bayes = s_bayes if s_bayes != None else 0
        min_value = max(s_base, s_bayes, s_bayes)
        if s_perple != None:
            min_value = max(min_value, s_perple)
        if s_base > 0:
            min_value = min(min_value, s_base)
        if s_bayes > 0:
            min_value = min(min_value, s_bayes)
        ...
        # Bayes 延迟
        lat_bayes = (3.0 / s_bayes) if s_bayes > 0 else p_value
        print(litmus, lat_bayes)
