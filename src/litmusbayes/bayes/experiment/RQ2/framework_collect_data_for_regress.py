...
class BestParamRunner(LitmusRunner):
    def __init__(
        ...
        regressed_tests = set()
        if not os.path.exists(regressed_list_path):
            raise FileNotFoundError(f"Regressed text file not found: {regressed_list_path}")

        with open(regressed_list_path, 'r') as f:
            for line in f:
                name = line.strip()
                if name:  # 忽略空行
                    regressed_tests.add(name)

        # 3. 构建任务队列 (Litmus, Vector, PredictedScore)
        self.todo_queue = []

        # 遍历 txt 中的测试用例构建任务
        for litmus_name in regressed_tests:
            ...
            while True:
                ...
                if vector not in fixed_vectors:
...
stat_log = "./log_record_init_banana_regress.log"
