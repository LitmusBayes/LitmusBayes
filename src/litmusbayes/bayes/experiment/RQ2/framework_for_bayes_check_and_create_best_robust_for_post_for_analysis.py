...
class PosteriorRFBO:
    def __init__(self, prior_bo: RandomForestBO, n_estimators=100):
        ...
        self.model = RandomForestRegressor(
            n_estimators=n_estimators,
            max_depth=5,  # 限制深度，防止对 400 条数据过拟合
            min_samples_leaf=4,  # 保证叶子节点的泛化性
            n_jobs=-1,
            random_state=SEED
        )
        self.X_post = []
...
class RobustPosteriorSelector(RobustParamSelector):
    ...
    def select_best_params(self, litmus_list, litmus_feature_map, alpha=2.0):
        ...
        return recommendations
# ================= 重写的 Main 函数 =================
class LitmusPredictionAnalyzer:
    def __init__(self, prior_bo, posterior_bo, alpha=1.0):
        self.prior_bo = prior_bo
        self.post_bo = posterior_bo
        self.alpha = alpha

    def get_tree_preds(self, model, X):
        """获取森林中所有树的预测值分布"""
        preds = [tree.predict(X) for tree in model.estimators_]
        return np.stack(preds).flatten()

    def analyze_bad_case(self, litmus_name, bad_param):
        print(f"\n{'=' * 20} 异常 Case 深度剖析 {'=' * 20}")
        print(f"Litmus: {litmus_name}")
        print(f"Bad Param Combo: {bad_param}")

        if litmus_name not in self.prior_bo.litmus_to_vector_dict:
            print("错误: 找不到该 Litmus 的特征向量！")
            return

        litmus_vec = self.prior_bo.litmus_to_vector_dict[litmus_name]
        feature = np.array([list(bad_param) + list(litmus_vec)])

        # 1. 拆解预测成分 (Log空间)
        prior_trees = self.get_tree_preds(self.prior_bo.model, feature)
        post_trees = self.get_tree_preds(self.post_bo.model, feature)

        prior_mean_log = np.mean(prior_trees)
        post_mean_log = np.mean(post_trees)
        post_std_log = np.std(post_trees)

        final_score_log = prior_mean_log + post_mean_log - (self.alpha * post_std_log)
        predicted_real_score = np.expm1(final_score_log)

        print("\n--- 1. 预测值拆解 (Log 空间) ---")
        print(f"先验模型预测均值 (Prior Log)   : {prior_mean_log:.4f}  -> 还原物理分数: {np.expm1(prior_mean_log):.4f}")
        print(f"残差模型预测均值 (Residual Log): {post_mean_log:.4f}  -> 带来的乘数效应: {np.exp(post_mean_log):.4f}x")
        print(f"残差模型不确定性 (Residual Std): {post_std_log:.4f}")
        print(f"LCB 稳健打分 (Alpha={self.alpha})    : {final_score_log:.4f}")
        print(f"最终预测实际表现 (Real Score)  : {predicted_real_score:.4f}")

        # 2. 检查残差模型中的最近邻 (Nearest Neighbors)
        # 这能告诉我们，残差模型是根据哪些真实数据产生了“幻觉”
        print("\n--- 2. 残差模型训练集最近邻分析 (KNN in Posterior Data) ---")
        if len(self.post_bo.X_post) == 0:
            print("残差模型没有训练数据！")
            return

        X_train_post = np.array(self.post_bo.X_post)
        y_train_residual = np.array(self.post_bo.y_residual)

        # 计算欧氏距离
        distances = pairwise_distances(feature, X_train_post)[0]
        top_k_indices = np.argsort(distances)[:5]

        for i, idx in enumerate(top_k_indices):
            dist = distances[idx]
            train_feat = X_train_post[idx]
            train_param = train_feat[:len(bad_param)]
            train_resid = y_train_residual[idx]

            # 尝试反推这是哪个 litmus (简单匹配向量)
            train_litmus_vec = train_feat[len(bad_param):]
            matched_litmus = "Unknown"
            for lname, lvec in self.prior_bo.litmus_to_vector_dict.items():
                if np.allclose(lvec, train_litmus_vec):
                    matched_litmus = lname
                    break

            print(f"  Top {i + 1} 邻居 (距离: {dist:.4f}):")
            print(f"    Litmus: {matched_litmus}")
            print(f"    参数  : {train_param.tolist()}")
            print(f"    该样本的真实残差 (Log) : {train_resid:.4f}")

        # 3. 诊断结论建议
        print("\n--- 3. 自动化诊断结论 ---")
        if np.expm1(prior_mean_log) > predicted_real_score * 2:
            print(
                "⚠️ 诊断: 先验模型严重过度自信！(Prior 过高)。这说明在旧芯片上这个组合表现很好，但在新芯片上残差模型没能把它拉回来（可能缺乏负样本）。")
        if post_mean_log > 0.5:
            print(
                "⚠️ 诊断: 残差模型出现了正向幻觉！(Residual 过高)。请检查上面的最近邻，大概率是这几个邻居带有极高的正残差，误导了当前预测。")
        if post_std_log > 1.0:
            print("⚠️ 诊断: 模型对该点的方差极大！目前的 Alpha 惩罚力度可能不足以抵消方差带来的风险，建议增大 Alpha。")
        print("========================================================\n")
...
if __name__ == "__main__":
    ...
    posterior_bo = PosteriorRFBO(prior_bo=bo, n_estimators=100)
    ...
    #     alpha=1.0  # 针对新芯片如果把握不大，可以将 alpha 调高到 1.5 增加保守性
    # )
    #
    # # 8. 统计与保存结果
    ...
    #
    # output_file = "best_params_recommendation_robust_3.json"
    ...
    analyzer = LitmusPredictionAnalyzer(bo, posterior_bo, alpha=1.0)
    analyzer.analyze_bad_case(
        litmus_name="MP+fence.w.w+ctrl",
        bad_param=[1, 3, 1, 0, 2, 1, 2, 2, 0, 1, 0] # 填入表现差的真实参数
    )