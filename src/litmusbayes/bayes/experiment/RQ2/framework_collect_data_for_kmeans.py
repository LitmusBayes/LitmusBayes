...
class BestParamRunner(LitmusRunner):
    def __init__(
            ...
            regressed_list_path,  # 新增：指定需要测试的 litmus txt 列表路径
            param_space,
            stat_log,
            logger=None,
            mode="time",
        ...
        self.ps = param_space
        self.logger = logger or get_logger(LOG_NAME)
        ...
        # 2. 读取需要测试的回归用例列表 txt
        self.logger.info(f"Loading regressed litmus list from {regressed_list_path}...")

        kmeans_tests = set()
        kmeans_path = "./best_representatives.json"
        if not os.path.exists(kmeans_path):
            raise FileNotFoundError(f"kmeans text file not found: {kmeans_path}")
        ...
        with open(kmeans_path, 'r', encoding='utf-8') as f:
            ...
            # 遍历字典中的每个子项（"0", "1", "2"...）
            for key in data:
                # 检查当前子项是否包含"representative"字段
                if "best_litmus_test" in data[key]:
                    # 将值添加到集合中（集合自动去重）
                    rep_value = data[key]["best_litmus_test"]
                    kmeans_tests.add(rep_value)

        # 3. 构建任务队列 (Litmus, Vector, PredictedScore)
        self.todo_queue = []

        # 遍历 txt 中的测试用例构建任务
        for litmus_name in kmeans_tests:
            info = self.recommendations[litmus_name]
            # 保持你原来的逻辑：这里强行给了硬编码的vec
            fixed_vectors = self.ps.get_bound_vector()
            for vec in fixed_vectors:
                pred_score = info.get('pred_score', -1.0)

                # 检查文件是否存在（防止 json/txt 里有文件名但磁盘上没有）
...
stat_log = "./log_record_init_banana_kmeans_2.log"
dir_path = "/home/whq/Desktop/code_list/perple_test/bayes_banana_new_log"
...
json_recommendation_path = "best_params_recommendation_robust_0.json"
# 新增：指定要跑的用例列表的文件
txt_regressed_path = "regressed_litmus.txt"
...
if __name__ == "__main__":
    ...
    logger.info(f"Start Validation Run | JSON source: {json_recommendation_path} | TXT filter: {txt_regressed_path}")

    param_space = LitmusParamSpace()

    # 实例化运行器
    runner = BestParamRunner(
        recommendation_json_path=json_recommendation_path,
        regressed_list_path=txt_regressed_path,  # 传入 TXT 路径
