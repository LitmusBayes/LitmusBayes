...
class PerLitmusRandomForestBO:
    def __init__(self, param_space: LitmusParamSpace, n_estimators=200):
        self.ps = param_space
        self.n_estimators = n_estimators
        self.logger = get_logger(LOG_NAME)

        # 核心改动：用字典保存多个模型和数据
        self.models = {}  # { litmus_name: RandomForestRegressor }
        self.train_data = defaultdict(lambda: {'X': [], 'y': []})

    def add(self, litmus_name, param_vec, score):
        # 核心改动：不再需要拼接 litmus_vec，只保存参数
        self.train_data[litmus_name]['X'].append(list(param_vec))
        self.train_data[litmus_name]['y'].append(score)

    def fit(self):
        self.logger.info(f"Start fitting {len(self.train_data)} independent models...")

        # 遍历每一个 litmus，单独训练一个随机森林
        for litmus, data in self.train_data.items():
            X = np.array(data['X'])
            y = np.array(data['y'])

            # 如果某个 litmus 数据太少，跳过不练
            if len(X) < 2:
                self.logger.warning(f"Skip training for {litmus} due to insufficient data (Samples: {len(X)})")
                continue

            # 动态调整叶子节点最小样本数，防止数据少于3条时报错
            leaf_samples = min(3, max(1, len(X) // 2))
            ...
            # 使用 log1p 防止 x=0 报错，同时压缩数值
            y_train_log = np.log1p(y)
            model.fit(X, y_train_log)

            # 保存训练好的模型
            self.models[litmus] = model

        self.logger.info(f"Successfully fitted {len(self.models)} models.")

    def predict_one(self, litmus_name, param_vec):
        if litmus_name not in self.models:
            return None
        model = self.models[litmus_name]
        pred_log = model.predict([list(param_vec)])[0]
        return np.expm1(pred_log)

    def predict_batch(self, litmus_list, param_list):
        """
        批量预测方法（按 litmus 路由给对应的模型）
        """
        # 将请求按 litmus 分组：{ litmus_name: [(original_index, param_vec), ...] }
        requests_per_litmus = defaultdict(list)
        for i, (litmus, param) in enumerate(zip(litmus_list, param_list)):
            if litmus in self.models:  # 只有在训练集里出现过（且成功训练出模型）的才能预测
                requests_per_litmus[litmus].append((i, list(param)))

        if not requests_per_litmus:
            return [], []

        result_map = {}

        # 针对每个模型进行批量预测，效率极高
        for litmus, reqs in requests_per_litmus.items():
            model = self.models[litmus]
            indices = [r[0] for r in reqs]
            X_batch = np.array([r[1] for r in reqs])

            pred_log = model.predict(X_batch)
            pred_expm1 = np.expm1(pred_log)

            # 将预测结果映射回原始数据的索引
            for idx, val in zip(indices, pred_expm1):
                result_map[idx] = val

        # 整理输出，确保顺序与输入数据的索引一致
        valid_indices = sorted(list(result_map.keys()))
        preds = [result_map[i] for i in valid_indices]

        return np.array(preds), valid_indices
# ================= 主程序 =================

# 配置路径
litmus_path = "../../litmus_test"
stat_log_base = "./non_contextual"
litmus_vec_path = "./litmus_vector_two_tower_fixed.log"
# litmus_vec_path = "./litmus_vector_emb.log"
# litmus_vec_path = "./litmus_vector_ngram_dnn.log"
# litmus_vec_path = "./litmus_vector_n_gram.log"
# litmus_vec_path = "./litmus_vector_gnn.log"
# litmus_vec_path = "./litmus_vector_n_gram.log"
...
if __name__ == "__main__":
    ...
    # 3. 初始化 BO
    # 3. 初始化新的 Multi-Model BO (不再需要 litmus_vec_path)
    param_space = LitmusParamSpace()
    bo = PerLitmusRandomForestBO(
        param_space,
        n_estimators=200
    )
    ...
    if total_count <= 10000:
        logger.warning("Data size <= 10000, splitting might be invalid based on request.")
    ...
    logger.info(f"Prediction finished in {time.time() - t0:.4f}s. Valid samples: {len(preds)}/{len(test_data)}")
    # =================================================================

    # 7. 将结果重新映射回 Groups 结构 (以下代码基本不用动)
    ...
    if total_litmus_cnt > 0:
        ...
        with open("../../results/contextual_results.txt", "a+") as f:
            f.write(f"non contextual : top1({top1_acc}), top3({top3_acc})\n")
