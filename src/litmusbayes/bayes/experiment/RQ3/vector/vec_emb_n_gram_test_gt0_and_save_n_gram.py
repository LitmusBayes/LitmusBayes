...
class RandomForestBO:
    def __init__(self, param_space: LitmusParamSpace, litmus_list, n_estimators=200,
                 litmus_vec_path="./raw_vector/litmus_vector.log"):
        self.ps = param_space
...
stat_log_base = "./n_gram"
litmus_vec_path = "./raw_vector/litmus_vector.log"
litmus_cycle_path = "./raw_vector/litmus_vector3.log"
cache_file_path = "../../../log/cache_norm.jsonl"


inference_text_path = "./raw_vector/litmus_vector3_aligned.log" # 请修改为你的新文件路径
inference_output_file = "litmus_vector_n_gram.log"
...
if __name__ == "__main__":
    ...
    gram_rho = analyze_embedding_quality(X_test, y_test, method_name="Raw N-gram Features(Test)")
    with open("../../results/rho_result.txt", "a+") as f:
        f.write(f"3-gram : {gram_rho}\n")
    print("=" * 60)
    # 6. 运行训练和推理
    ...
    # 评估测试集 (关键指标)
    gram_dnn_rho = analyze_embedding_quality(emb_test, y_test, method_name="Test Set (Unseen)")
    with open("../../results/rho_result.txt", "a+") as f:
        f.write(f"3-gram + DNN : {gram_dnn_rho}\n")
    print("=" * 60)
    print("=" * 60)
