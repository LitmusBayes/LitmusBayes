...
static void *P0(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    "#END _litmus_P0\n\t"
    :[x5] "=&r" (out_0_x5[_i])
    :[x6] "r" (&_a->x[_j]),[x8] "r" (&_a->y[_j]), [x7] "r" (trashed_x7)
    :"cc","memory"
    );trashed_x7 += 1;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      :[x6] "r" (&_a->y[_j]),[x8] "r" (&_a->z[_j]),[x10] "r" (&_a->x[_j]), [x9] "r" (trashed_x9)
      :"cc","memory"
      );trashed_x9 += 1;
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/LB+fence.r.rw+data-po.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
