...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P1\n\t"
      :[x5] "=&r" (out_1_x5[_i])
      :[x6] "r" (&_a->y[_j]),[x7] "r" (x7_val),[x8] "r" (&_a->z[_j])
      :"cc","memory"
      );x7_val += 1;
...
REVERSE_CTX(ctx.out_2_x5, _b->size_of_test);
REVERSE_CTX(ctx.out_2_x7, _b->size_of_test);
int counter_heuristic = 0;
for (int perple_c = 0; perple_c <= _b->size_of_test - 1 ; perple_c++){
  if (perple_c< _b->size_of_test && ctx.out_1_x5[(ctx.out_2_x5[perple_c] - 1) / 1]>=1*((ctx.out_2_x7[perple_c] - 1) / 1 + 1)+1 && (ctx.out_2_x5[perple_c] - 1) / 1< _b->size_of_test){
    counter_heuristic += 1;
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/ISA2+fence.rw.rw+fence.rw.rw+ctrlfencei.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA2+fence.rw.rw+fence.rw.rw+ctrlfencei");
  fprintf(out,"%s\n","\"Fence.rw.rwdWW Rfe Fence.rw.rwdRW Rfe DpCtrlFenceIdR Fre\"");
  fprintf(out,"%s\n","{0:x5=1; 0:x6=x; 0:x7=y; 1:x6=y; 1:x7=1; 1:x8=z; 2:x6=z; 2:x8=x;}");
  fprintf(out,"%s\n"," P0          | P1          | P2             ;");
