...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.rw.rw+ctrl-cleaninit, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.rw.rw+ctrl-cleaninit, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.rw.rw+ctrl-cleaninit");
  check_globals(_a);
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P0\n\t"
      :
      :[x5] "r" (x5_val),[x6] "r" (&_a->x[_j]),[x7] "r" (x7_val),[x8] "r" (&_a->y[_j])
      :"cc","memory"
      );x5_val += 1;
...
#else
#include "MP+fence.rw.rw+ctrl-cleaninit.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/MP+fence.rw.rw+ctrl-cleaninit.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV MP+fence.rw.rw+ctrl-cleaninit");
  fprintf(out,"%s\n","{0:x6=x; 0:x8=y; 0:x5=1; 0:x7=1; 1:x6=y; 1:x8=x;}");
  fprintf(out,"%s\n"," P0          | P1             ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.rw.rw+ctrl-cleaninit Allowed\n");
  ...
  fprintf(out,"Hash=b97a4d0e691834e6bdef7e13399f1c07\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+fence.rw.rw+ctrl-cleaninit %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  if (p_true > 0) {
  ...
  fprintf(out,"Time MP+fence.rw.rw+ctrl-cleaninit %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.rw.rw+ctrl-cleaninit: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
/* fatal("MP+fence.rw.rw+ctrl-cleaninit, sum_hist") */
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_rw_2E_rw_2B_ctrl_2D_cleaninit(int argc, char **argv, FILE *out) {
