...
static void *P0(void *_vb) {
  ...
  uint64_t *out_0_x7 = _a->out_0_x7;
  uint64_t *out_0_x8 = _a->out_0_x8;
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "ld %[x8],0(%[x9])\n"
      "#END _litmus_P0\n\t"
      :[x8] "=&r" (out_0_x8[_i]),[x7] "=&r" (out_0_x7[_i])
      :[x5] "r" (x5_val),[x6] "r" (&_a->x[_j]),[x9] "r" (&_a->y[_j])
      :"cc","memory"
      );x5_val += 1;
...
static void *P1(void *_vb) {
  ...
  int _size_of_test = _a->_p->size_of_test;
  int _stride = _a->_p->stride;
  uint64_t *out_1_x7 = _a->out_1_x7;
  uint64_t *out_1_x8 = _a->out_1_x8;
  for (int _j = _stride ; _j > 0 ; _j--) {
    int x5_val = 1 ;
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "ld %[x8],0(%[x9])\n"
      "#END _litmus_P1\n\t"
      :[x8] "=&r" (out_1_x8[_i]),[x7] "=&r" (out_1_x7[_i])
...
REVERSE_CTX(ctx.out_0_x8, _b->size_of_test);
REVERSE_CTX(ctx.out_1_x8, _b->size_of_test);
int counter_heuristic = 0;
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  if (perple_a< _b->size_of_test && ctx.out_1_x8[(ctx.out_0_x8[perple_a] - 1) / 1 + 1]<=1*(perple_a-1)+1 && (ctx.out_0_x8[perple_a] - 1) / 1 + 1< _b->size_of_test){
    counter_heuristic += 1;
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/SB+rfi-pos.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV SB+rfi-pos");
