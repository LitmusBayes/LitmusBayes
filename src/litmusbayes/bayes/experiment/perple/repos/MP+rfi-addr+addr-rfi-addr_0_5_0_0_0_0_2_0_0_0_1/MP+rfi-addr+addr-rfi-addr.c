...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x13],0(%[x15])\n"
      "#END _litmus_P1\n\t"
      :[x13] "=&r" (out_1_x13[_i]),[x11] "=&r" (out_1_x11[_i]),[x5] "=&r" (out_1_x5[_i]),[x15] "=&r" (trashed_x15),[x12] "=&r" (trashed_x12),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
      :[x6] "r" (&_a->y[_j]),[x8] "r" (x8_val),[x9] "r" (&_a->z[_j]),[x14] "r" (&_a->x[_j])
      :"cc","memory"
      );x8_val += 1;
...
for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
  if (ctx.out_1_x13[perple_b]<=1*((ctx.out_1_x5[perple_b] - 1) / 1-1)+1 && perple_b< _b->size_of_test){
    counter_heuristic += 1;
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/MP+rfi-addr+addr-rfi-addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
