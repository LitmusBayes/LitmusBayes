...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x13],0(%[x14])\n"
      "#END _litmus_P1\n\t"
      :[x13] "=&r" (out_1_x13[_i]),[x12] "=&r" (out_1_x12[_i]),[x8] "=&r" (out_1_x8[_i]),[x5] "=&r" (out_1_x5[_i]),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
      :[x6] "r" (&_a->y[_j]),[x9] "r" (&_a->z[_j]),[x11] "r" (x11_val),[x14] "r" (&_a->x[_j])
      :"cc","memory"
      );x11_val += 1;
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/MP+fence.rw.rw+addr-fri-rfi-ctrl.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
