...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      :[x5] "r" (x5_val),[x6] "r" (&_a->z[_j]),[x8] "r" (&_a->a[_i]),[x11] "r" (&_a->x[_j])
      :"cc","memory"
      );x5_val += 1;
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  if (perple_a< _b->size_of_test && ctx.out_1_x10[(ctx.out_0_x10[perple_a] - 1) / 1 + 1]<=1*(perple_a-1)+1 && (ctx.out_0_x10[perple_a] - 1) / 1 + 1< _b->size_of_test){
    counter_heuristic += 1;
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/SB+po-addrs.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV SB+po-addrs");
