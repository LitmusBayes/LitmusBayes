...
inline static int final_cond(int _out_0_x7,int _out_0_x9,int _out_1_x7,int _out_1_x9) {
  switch (_out_0_x7) {
  case 1:
    switch (_out_0_x9) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%i; 0:x9=%i; 1:x7=%i; 1:x9=%i;\n",c,show ? '*' : ':',(int)o[out_0_x7_f],(int)o[out_0_x9_f],(int)o[out_1_x7_f],(int)o[out_1_x9_f]);
}
...
static void *P0(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x9],0(%[x11])\n"
      "#END _litmus_P0\n\t"
      :[x9] "=&r" (out_0_x9[_i]),[x7] "=&r" (out_0_x7[_i]),[x11] "=&r" (trashed_x11),[x8] "=&r" (trashed_x8)
      :[x5] "r" (x5_val),[x6] "r" (&_a->x[_j]),[x10] "r" (&_a->y[_j])
      :"cc","memory"
      );x5_val += 1;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x9],0(%[x11])\n"
      "#END _litmus_P1\n\t"
      :[x9] "=&r" (out_1_x9[_i]),[x7] "=&r" (out_1_x7[_i]),[x11] "=&r" (trashed_x11),[x8] "=&r" (trashed_x8)
      :[x5] "r" (x5_val),[x6] "r" (&_a->y[_j]),[x10] "r" (&_a->x[_j])
      :"cc","memory"
      );x5_val += 1;
...
REVERSE_CTX(ctx.out_0_x9, _b->size_of_test);
REVERSE_CTX(ctx.out_1_x9, _b->size_of_test);
int counter_heuristic = 0;
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  if (perple_a< _b->size_of_test && ctx.out_1_x9[(ctx.out_0_x9[perple_a] - 1) / 1 + 1]<=1*(perple_a-1)+1 && (ctx.out_0_x9[perple_a] - 1) / 1 + 1< _b->size_of_test){
    counter_heuristic += 1;
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/SB+rfi-addrs.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV SB+rfi-addrs");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  ...
  fprintf(out,"Hash=d1f48b23c60225caafe9c771305dfca2\n");
  fprintf(out,"Cycle=Rfi DpAddrdR Fre Rfi DpAddrdR Fre\n");
  fprintf(out,"Relax SB+rfi-addrs %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
