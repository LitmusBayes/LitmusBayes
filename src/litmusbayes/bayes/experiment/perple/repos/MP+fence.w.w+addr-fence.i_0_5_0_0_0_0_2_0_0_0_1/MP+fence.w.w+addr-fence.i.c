...
static void *P0(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    "#END _litmus_P0\n\t"
    :
    :[x6] "r" (&_a->x[_j]),[x8] "r" (&_a->y[_j]), [x7] "r" (trashed_x7), [x5] "r" (trashed_x5)
    :"cc","memory"
    );trashed_x5 += 1;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x11],0(%[x12])\n"
      "#END _litmus_P1\n\t"
      :[x11] "=&r" (out_1_x11[_i]),[x5] "=&r" (out_1_x5[_i]),[x10] "=&r" (trashed_x10),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7)
      :[x6] "r" (&_a->y[_j]),[x9] "r" (&_a->z[_j]),[x12] "r" (&_a->x[_j])
      :"cc","memory"
      );mbar();
    }
  }
...
REVERSE_CTX(ctx.out_1_x11, _b->size_of_test);
int counter_heuristic = 0;
for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
  if (ctx.out_1_x11[perple_b]<=1*((ctx.out_1_x5[perple_b] - 1) / 1-1)+1 && perple_b< _b->size_of_test){
    counter_heuristic += 1;
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/MP+fence.w.w+addr-fence.i.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
