...
inline static int final_cond(int _out_1_x10,int _out_1_x11,int _out_1_x13) {
  switch (_out_1_x10) {
  case 1:
    switch (_out_1_x11) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x10=%i; 1:x11=%i; 1:x13=%i;\n",c,show ? '*' : ':',(int)o[out_1_x10_f],(int)o[out_1_x11_f],(int)o[out_1_x13_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.w.w+fri-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.w.w+fri-rfi-addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.w.w+fri-rfi-addr");
  check_globals(_a);
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P0\n\t"
      :
      :[x5] "r" (x5_val),[x6] "r" (&_a->y[_j]),[x9] "r" (&_a->x[_j])
      :"cc","memory"
      );x5_val += 2;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x13],0(%[x9])\n"
      "#END _litmus_P1\n\t"
      :[x13] "=&r" (out_1_x13[_i]),[x11] "=&r" (out_1_x11[_i]),[x10] "=&r" (out_1_x10[_i]),[x9] "=&r" (trashed_x9),[x8] "=&r" (trashed_x8)
      :[x6] "r" (&_a->y[_j]),[x7] "r" (x7_val),"[x9]" (&_a->x[_j])
      :"cc","memory"
      );x7_val += 2;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x10_i,_out_1_x11_i,_out_1_x13_i));
...
for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
  if (ctx.out_1_x10[perple_b] % 2 == 1 && ctx.out_1_x13[perple_b] % 2 == 1 && ctx.out_1_x13[perple_b]<=2*((ctx.out_1_x10[perple_b] - 1) / 2-1)+1 && perple_b< _b->size_of_test){
    counter_heuristic += 1;
  }
}
...
#else
#include "MP+fence.w.w+fri-rfi-addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/MP+fence.w.w+fri-rfi-addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV MP+fence.w.w+fri-rfi-addr");
  fprintf(out,"%s\n","{0:x5=1; 0:x9=x; 0:x6=y; 1:x7=2; 1:x6=y; 1:x9=x;}");
  fprintf(out,"%s\n"," P0          | P1             ;");
  fprintf(out,"%s\n"," sw x5,0(x9) | lw x10,0(x6)   ;");
  fprintf(out,"%s\n"," fence w,w   | sw x7,0(x6)    ;");
  fprintf(out,"%s\n"," sw x5,0(x6) | lw x11,0(x6)   ;");
  fprintf(out,"%s\n","             | xor x8,x11,x11 ;");
  fprintf(out,"%s\n","             | add x9,x9,x8   ;");
  fprintf(out,"%s\n","             | lw x13,0(x9)   ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists (1:x10=1 /\\ 1:x11=2 /\\ 1:x13=0)");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.w.w+fri-rfi-addr Allowed\n");
  ...
  fprintf(out,"Time MP+fence.w.w+fri-rfi-addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.w.w+fri-rfi-addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
/* fatal("MP+fence.w.w+fri-rfi-addr, sum_hist") */
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_w_2E_w_2B_fri_2D_rfi_2D_addr(int argc, char **argv, FILE *out) {
