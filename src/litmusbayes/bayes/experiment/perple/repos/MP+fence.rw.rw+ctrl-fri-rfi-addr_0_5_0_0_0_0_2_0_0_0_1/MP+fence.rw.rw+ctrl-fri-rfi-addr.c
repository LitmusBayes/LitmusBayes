...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x12],0(%[x14])\n"
      "#END _litmus_P1\n\t"
      :[x12] "=&r" (out_1_x12[_i]),[x10] "=&r" (out_1_x10[_i]),[x7] "=&r" (out_1_x7[_i]),[x5] "=&r" (out_1_x5[_i]),[x14] "=&r" (trashed_x14),[x11] "=&r" (trashed_x11)
      :[x6] "r" (&_a->y[_j]),[x8] "r" (&_a->z[_j]),[x9] "r" (x9_val),[x13] "r" (&_a->x[_j])
      :"cc","memory"
      );x9_val += 1;
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/MP+fence.rw.rw+ctrl-fri-rfi-addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
