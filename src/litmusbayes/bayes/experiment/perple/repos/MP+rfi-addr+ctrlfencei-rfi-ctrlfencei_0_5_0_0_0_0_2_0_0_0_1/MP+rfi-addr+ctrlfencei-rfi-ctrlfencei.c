...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x10],0(%[x11])\n"
      "#END _litmus_P1\n\t"
      :[x10] "=&r" (out_1_x10[_i]),[x9] "=&r" (out_1_x9[_i]),[x5] "=&r" (out_1_x5[_i])
      :[x6] "r" (&_a->y[_j]),[x7] "r" (x7_val),[x8] "r" (&_a->z[_j]),[x11] "r" (&_a->x[_j])
      :"cc","memory"
      );x7_val += 1;
...
for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
  if (ctx.out_1_x10[perple_b]<=1*((ctx.out_1_x5[perple_b] - 1) / 1-1)+1 && perple_b< _b->size_of_test){
    counter_heuristic += 1;
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/MP+rfi-addr+ctrlfencei-rfi-ctrlfencei.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
