...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("A, global y unstabilized") ;
      }
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for ./make_new_litmus/litmus_output/A.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV A");
  fprintf(out,"%s\n","\"Coe PodWW Coe RfLeave PodRR FrBack\"");
  fprintf(out,"%s\n","{0:x6=x; 0:x8=y; 1:x6=y; 1:x8=x; 2:x6=y; 2:x8=x;}");
  fprintf(out,"%s\n"," P0          | P1          | P2          ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  ...
  fprintf(out,"Hash=e34877139923a16c5cf7ce5cd938c9f5\n");
  fprintf(out,"Generator=diyone7 (version 7.56)\n");
  fprintf(out,"Com=Co Co Fr\n");
  fprintf(out,"Orig=Coe PodWW Coe RfLeave PodRR FrBack\n");
