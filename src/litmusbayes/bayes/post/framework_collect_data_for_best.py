...
class BestParamRunner(LitmusRunner):
    def __init__(
        ...
        for litmus_name, info in self.recommendations.items():
            # [FIX] 兼容逻辑：判断 info 是列表还是字典
            if isinstance(info, list):
                # 情况 A：JSON 值直接是参数列表 (你现在的 JSON 格式)
                vec = info
                pred_score = -1.0  # 这种格式没有预测分
            elif isinstance(info, dict):
                # 情况 B：JSON 值是包含 'param' 的字典 (代码原本期望的格式)
                vec = info.get('param')
                pred_score = info.get('pred_score', -1.0)
            else:
                self.logger.warning(f"Unknown format for {litmus_name}: {type(info)}")
                continue
        ...
        self.cache = ResultCache(stat_log + ".validation_cache_banana.jsonl")
...
stat_log = "/home/whq/Desktop/code_list/perple_test/bayes_stat/log_record_banaba_best.log"
...
json_recommendation_path = "./posterior_best_params.json"
